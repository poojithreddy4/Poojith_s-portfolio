...
const MobileNav = ({ closeNav, showNav }: Props) => {
  ...
  return (
    <div>
      <div
        className={`fixed ${navOpen} transform transition-all duration-500 insert-0 z-[1000] bg-black opacity-70 w-full h-screen`}
      ></div>
      <div
      ...
      >
        ...
        <CgClose
          onClick={closeNav}
          className="absolute top-[0.7rem] right-[1.4rem] sm:h-8 sm:w-8 w-6 h-6 text-white"
        />
      </div>
    </div>
  );
